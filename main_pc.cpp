...
typedef   vector<ratslam::PosecellNetwork*>  pcn;

ratslam_ros::TopologicalAction pc_output;
void odo_callback(nav_msgs::OdometryConstPtr odo, pcn pc, ros::Publisher * pub_pc)
{
...
int main(int argc, char * argv[])
{
  ...
  ratslam::PosecellNetwork * pc_t[4];

  for(int i=0;i<4;i++){
	  pc_t[i]=new ratslam::PosecellNetwork(ratslam_settings);
  }

  vector<ratslam::PosecellNetwork*> pc_v;

  for(int i=0;i<4;i++)
	  pc_v.push_back(pc_t[i]);
  ...
  ros::Subscriber sub_odometry = node.subscribe<nav_msgs::Odometry>("/odom", 1000, boost::bind(odo_callback,_1, pc_v, &pub_pc), ros::VoidConstPtr(),
